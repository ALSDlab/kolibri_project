...
io.on('connection', (socket) => {
    ...
    socket.on('offer', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for offer' });
                return;
            }
    ...
    socket.on('answer', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for answer' });
                return;
            }
            const { targetPeerId, sdp } = data;
    ...
    socket.on('ice-candidate', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for ICE candidate' });
                return;
            }
    ...
    socket.on('signal', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for signal' });
                return;
            }
    ...
    socket.on('hang-up', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for hang-up' });
                return;
            }
            ...
            socket.emit('error', { message: 'Failed to hang up' });
        }
    });
    ...
    socket.on('call-refusal', (data) => {
        try {
            if (!data || !data.targetPeerId) {
                socket.emit('error', { message: 'Target peer ID is required for call refusal' });
                return;
            }
            ...
            socket.emit('error', { message: 'Failed to refuse call' });
        }
    });
