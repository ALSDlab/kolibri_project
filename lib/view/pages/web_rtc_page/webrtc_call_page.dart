...
class WebrtcCallPage extends StatefulWidget {
  final WebRTCViewModel viewModel; // Passed from WebRTCMainView

  const WebrtcCallPage({super.key, required this.viewModel});
...
class _WebrtcCallPageState extends State<WebrtcCallPage> {
  ...
  void initState() {
    super.initState();
    widget.viewModel.setCallViewContext(context); // For programmatic pop
  ...
  void dispose() {
    ...
    widget.viewModel.clearCallViewContext();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<WebRTCViewModel>(
      builder: (context, viewModel, child) {
        // If call ended (e.g. remote peer hung up), ViewModel state might change.
        // The endCall method in ViewModel should pop this view.
        // This is a fallback / alternative to WillPopScope if state drives navigation entirely.
        // if (viewModel.state.screenState != AppScreenState.inCall && mounted) {
